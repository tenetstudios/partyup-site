...
export const FLOAT_REALTIME_PROTOCOL_VERSION = 1 as const;
...
export type FloatRealtimeActionType =
  | "PLACE_WALL"
  | "REMOVE_WALL"
  | "PLACE_NAILS"
  | "REMOVE_NAILS"
...
export type FloatTimelineInsertResult =
  | { status: "applied"; rewound: boolean; result: GameActionResult }
  | { status: "queued"; rewound: false }
  | { status: "duplicate"; rewound: false }
  | { status: "too_old" | "too_far_ahead"; rewound: false }
  | { status: "rejected"; rewound: boolean; result: GameActionResult };

export function simulationTimeMsToTick(simulationTimeMs: number) {
  if (!Number.isFinite(simulationTimeMs) || simulationTimeMs < 0) throw new Error("Invalid Float simulation time");
  return Math.floor(simulationTimeMs * FLOAT_SIMULATION_HZ / 1_000);
}

export function simulationTickToTimeMs(simulationTick: number) {
  if (!Number.isSafeInteger(simulationTick) || simulationTick < 0) throw new Error("Invalid Float simulation tick");
  return simulationTick * 1_000 / FLOAT_SIMULATION_HZ;
}

export function compareFloatRealtimeActions(left: FloatRealtimeAction, right: FloatRealtimeAction) {
...
export function floatActorTopic(matchId: string, actorPlayerId: FloatPlayerId) {
  if (!matchId || !/^[0-9a-f-]{36}$/i.test(matchId)) throw new Error("Invalid Float match ID");
...
function stringValue(value: unknown, name: string) {
  if (typeof value !== "string" || value.length === 0 || value.length > 240) throw new Error(`Invalid ${name}`);
  return value;
}

export function validateFloatRealtimeAction(
  value: unknown,
  expected: { matchId: string; actorPlayerId: FloatPlayerId },
): FloatRealtimeAction {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Invalid Float action envelope");
  const action = value as Record<string, unknown>;
  if (action.protocolVersion !== FLOAT_REALTIME_PROTOCOL_VERSION) throw new Error("Float protocol update required");
  if (action.matchId !== expected.matchId) throw new Error("Float action match mismatch");
  if (action.actorPlayerId !== expected.actorPlayerId) throw new Error("Float action actor mismatch");
  if (typeof action.actionId !== "string" || !/^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(action.actionId)) throw new Error("Invalid Float action ID");
  integer(action.clientSequence, "client sequence");
  integer(action.simulationTick, "simulation tick");
  if (Number(action.clientSequence) < 1 || Number(action.simulationTick) < 0) throw new Error("Invalid Float action ordering");
  const allowed = new Set<FloatRealtimeActionType>(["PLACE_WALL", "REMOVE_WALL", "PLACE_NAILS", "REMOVE_NAILS", "PLACE_GLUE", "REMOVE_GLUE", "REPAIR_WALL", "SEND_BALLOON", "POP_BALLOON"]);
  if (typeof action.actionType !== "string" || !allowed.has(action.actionType as FloatRealtimeActionType)) throw new Error("Unsupported Float action");
  if (!action.payload || typeof action.payload !== "object" || Array.isArray(action.payload)) throw new Error("Invalid Float action payload");
  if (new TextEncoder().encode(JSON.stringify(action.payload)).byteLength > FLOAT_MAX_PAYLOAD_BYTES) throw new Error("Float action payload is too large");
  return action as FloatRealtimeAction;
}

export function floatRealtimeActionToCoreAction(state: FloatMatchState, envelope: FloatRealtimeAction): FloatMatchAction {
  ...
  if (envelope.actionType === "PLACE_WALL") {
    ...
    return { type: "PLACE_WALL", actorPlayerId: envelope.actorPlayerId, wall: createWallSegment(room.id, orientation as WallOrientation, integer(payload.gridX, "wall grid X"), integer(payload.gridY, "wall grid Y")) };
  }
  if (envelope.actionType === "POP_BALLOON") return { type: "POP_BALLOON", actorPlayerId: envelope.actorPlayerId, balloonId: stringValue(payload.balloonId, "balloon") };
  if (envelope.actionType === "SEND_BALLOON") {
    ...
    return { type: "SEND_BALLOON", actorPlayerId: envelope.actorPlayerId, targetPlayerId: envelope.actorPlayerId === "playerA" ? "playerB" : "playerA", balloonType: balloonType as BalloonType, lane: lane as SpawnLane, sentAt: simulationTickToTimeMs(envelope.simulationTick) };
  }
  return { type: envelope.actionType, actorPlayerId: envelope.actorPlayerId, wallSegmentId: stringValue(payload.wallSegmentId, "wall segment") } as FloatMatchAction;
}

type TimelineCheckpoint = { simulationTick: number; state: FloatMatchState };

export class FloatRealtimeTimeline {
  state: FloatMatchState;
  currentTick: number;
  private checkpoints: TimelineCheckpoint[];
  private journal: FloatRealtimeAction[] = [];
  private knownActionIds = new Set<string>();
  private appliedActionIds = new Set<string>();

  constructor(initialState: FloatMatchState, initialTick = simulationTimeMsToTick(initialState.simulationTimeMs)) {
  ...
  advanceTo(targetTick: number) {
    if (!Number.isSafeInteger(targetTick) || targetTick < this.currentTick) throw new Error("Timeline cannot advance backward");
    const updates = [];
    while (this.currentTick < targetTick && this.state.status === "active") {
      ...
      if (this.currentTick % FLOAT_CHECKPOINT_INTERVAL_TICKS === 0) this.saveCheckpoint();
    }
    return updates;
  }

  insert(action: FloatRealtimeAction): FloatTimelineInsertResult {
    if (this.knownActionIds.has(action.actionId)) return { status: "duplicate", rewound: false };
    if (action.simulationTick < this.currentTick - FLOAT_REWIND_HISTORY_TICKS) return { status: "too_old", rewound: false };
    if (action.simulationTick > this.currentTick + FLOAT_MAX_FUTURE_TICKS) return { status: "too_far_ahead", rewound: false };
  ...
  private rebuildThrough(targetTick: number, rewindTick: number) {
    ...
    const results = new Map<string, GameActionResult>();
    while (this.currentTick < targetTick && this.state.status === "active") {
  ...
  private applyActionsAtCurrentTick(results = new Map<string, GameActionResult>()) {
    for (const action of this.journal) {
      if (action.simulationTick !== this.currentTick || this.appliedActionIds.has(action.actionId)) continue;
...
export function canonicalFloatJson(value: unknown): string {
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new Error("Float hash state contains a non-finite number");
    return JSON.stringify(Object.is(value, -0) ? 0 : Number(value.toFixed(9)));
  }
  if (value === null || typeof value !== "object") return JSON.stringify(value);
  if (Array.isArray(value)) return `[${value.map(canonicalFloatJson).join(",")}]`;
  const record = value as Record<string, unknown>;
  return `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${canonicalFloatJson(record[key])}`).join(",")}}`;
}

export function floatHashCoordinateKey(coordinates: FloatHashCoordinates) {
...
export async function hashFloatState(coordinates: FloatHashCoordinates, state: FloatMatchState) {
...
export type FloatSequenceReceiveResult = {
  ...
  duplicate: boolean;
  missing: { fromSequence: number; toSequence: number } | null;
};

export class FloatSequenceInbox {
  private throughSequence: number;
  private buffered = new Map<number, FloatRealtimeAction>();

  constructor(throughSequence = 0) {
    if (!Number.isSafeInteger(throughSequence) || throughSequence < 0) throw new Error("Invalid Float sequence cursor");
    this.throughSequence = throughSequence;
  }

  receive(action: FloatRealtimeAction): FloatSequenceReceiveResult {
    if (action.clientSequence <= this.throughSequence || this.buffered.has(action.clientSequence)) return { ready: [], duplicate: true, missing: null };
    if (action.clientSequence - this.throughSequence > FLOAT_MAX_SEQUENCE_GAP) throw new Error("Float action sequence gap is too large");
    this.buffered.set(action.clientSequence, action);
    const ready: FloatRealtimeAction[] = [];
