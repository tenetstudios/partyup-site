  ...
  return <section className="mt-6 rounded-xl border border-fuchsia-400/25 bg-fuchsia-950/10 p-5">
    ...
    {!game ? <button type="button" disabled={busy || roomEnded} onClick={() => void run(() => startWildGame(supabase, roomId))} className="mt-5 rounded-lg bg-fuchsia-600 px-5 py-3 font-black disabled:opacity-50">{busy ? "Starting…" : "Start Into the Wild"}</button> : <>
      ...
      {game.status === "active" && !roomEnded && <div className="mt-6 border-t border-white/10 pt-5">
        <h3 className="font-black">Launch Wild Mission</h3>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <label className="text-sm font-bold text-zinc-300 md:col-span-2">Target scope<select value={scope} onChange={(event) => { const value = event.target.value as typeof scope; setScope(value); if (value === "squad" && verification === "none") setVerification("match_faction"); if (value === "faction" && verification === "form_squad") setVerification("none"); }} className="mt-1 w-full rounded bg-black p-3 text-white"><option value="faction">Individual / faction (existing)</option><option value="squad">Squad</option></select></label>
          ...
          <label className="text-sm font-bold text-zinc-300 md:col-span-2">Mission type<select value={verification} onChange={(event) => { const value = event.target.value as typeof verification; setVerification(value); if (value === "form_squad") { setScope("squad"); setRequiredProgress("1"); } if (value === "match_faction") { setRequiredMatches("2"); setDuration("20"); } if (value === "live_node") setRequiredProgress("1"); }} className="mt-1 w-full rounded bg-black p-3 text-white">{scope === "faction" && <option value="none">Manual completion</option>}{scope === "squad" && <option value="form_squad">Form a squad</option>}<option value="same_faction">Find same faction</option><option value="different_faction">Find different faction</option><option value="specific_faction">Find specific faction</option><option value="memory_upload">Memory upload</option><option value="match_faction">Match opposing faction players</option>{scope === "squad" && <option value="live_node">Live Node claim</option>}</select></label>
          {scope === "squad" && verification !== "form_squad" && <label className="text-sm font-bold text-zinc-300">Required aggregate progress<input type="number" min={1} max={20} value={requiredProgress} onChange={(event) => setRequiredProgress(event.target.value)} className="mt-1 w-full rounded bg-black p-3 text-white" /></label>}
