            ...
            {state.mission ? <>
              ...
              {state.mission.config.verification_type === "memory_upload" ? (
                ...
                <div className="mt-4">
                  ...
                  {encounterState?.eligible ? <p className="mt-2 text-3xl font-black text-fuchsia-300">{Math.min(encounterState.progress, encounterState.required_encounters)} / {encounterState.required_encounters}</p> : <p className="mt-2 text-sm font-black text-amber-300">You can help an eligible player by showing your QR.</p>}
                  {encounterState?.completed ? <p className="mt-4 font-black text-emerald-300">VERIFIED ✓ Influence awarded.</p> : <div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={() => setEncounterMode("qr")} className="rounded-lg bg-purple-600 px-5 py-3 font-black">Show My QR</button>{encounterState?.eligible && <button type="button" onClick={() => setEncounterMode("code")} className="rounded-lg bg-fuchsia-600 px-5 py-3 font-black">Enter Player Code</button>}</div>}
                  ...
                  {encounterMode === "code" && encounterState?.eligible && <div className="mt-5 max-w-sm"><input value={encounterCode} onChange={(event) => setEncounterCode(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === "Enter") void redeemEncounter(); }} maxLength={64} placeholder="F7K2A1B9" className="w-full rounded bg-black px-4 py-3 text-center text-xl font-black uppercase tracking-[0.2em] text-white" /><button type="button" onClick={() => void redeemEncounter()} disabled={busy || !encounterCode.trim()} className="mt-3 w-full rounded bg-fuchsia-600 px-5 py-3 font-black disabled:opacity-50">{busy ? "Checking…" : "Verify Encounter"}</button></div>}
              ...
              ) : !state.mission.eligible ? <p className="mt-4 text-sm font-black text-amber-300">This objective belongs to another faction.</p> : state.mission.viewer_completed ? <p className="mt-4 font-black text-emerald-300">✓ Mission complete. Influence added.</p> : <button type="button" onClick={() => void complete()} disabled={busy} className="mt-4 rounded-lg bg-fuchsia-600 px-5 py-3 font-black disabled:opacity-50">{busy ? "Completing…" : "Complete Mission"}</button>}
