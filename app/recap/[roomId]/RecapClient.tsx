...
export default function RecapClient({ roomId }: { roomId: string }) {
  ...
  return <main className="min-h-screen bg-[#05040b] text-white"><HomeHeader />
    ...
    <div className="mx-auto max-w-6xl px-5 py-10 md:py-14">{error && <p className="mb-6 rounded-md border border-amber-300/20 bg-amber-950/30 p-4 text-sm font-bold text-amber-100">{error}</p>}
      <section><p className={partyUpTheme.sectionLabel}>From the room</p><h2 className="mt-2 text-3xl font-black">Memories</h2>{memories.length === 0 ? <p className="mt-5 text-sm font-semibold text-[#aaa4b8]">No Memories from this event are available now.</p> : <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-5">{memories.map((memory) => { const url = getMemoryPublicUrl(supabase, memory.media_path); return <article key={memory.id} className={`${partyUpTheme.glassCard} overflow-hidden`}><div className="aspect-square bg-[#070712]">{memory.media_type === "image" ? <img src={url} alt="" className="h-full w-full object-cover" /> : <video src={url} className="h-full w-full object-cover" controls preload="metadata" />}</div><div className="flex items-center justify-between gap-3 p-3 md:p-4"><div className="min-w-0"><p className="truncate text-xs font-black md:text-sm">{memory.uploader_name || "Guest"}</p><p className={`mt-1 truncate text-[11px] font-bold ${partyUpTheme.textMuted}`}>{formatMemoryTimestamp(memory.created_at)}</p></div><button type="button" disabled={processingId === memory.id} onClick={() => void toggleSaved(memory)} className={`${memory.is_saved ? `${partyUpTheme.tabActive} border` : partyUpTheme.ghostButton} shrink-0 rounded-md px-3 py-2 text-xs font-black`}>{memory.is_saved ? "Saved" : "Save"}</button></div></article>; })}</div>}</section>
